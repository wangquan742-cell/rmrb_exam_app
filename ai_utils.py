...
2. 每个 Word 文档内容包括：原文关键内容精读、深层含义、结构拆解、申论积累、面试思维、机关思维、工业视角、行测言语、中英表达、金句积累。
...
标题｜原文关键内容｜深层含义｜结构拆解｜申论主题｜申论规范表达｜面试题｜面试答题思路｜机关思维｜工业视角积累｜行测言语积累｜中英表达｜金句积累
    ...
    return pd.DataFrame([
        {
            ...
            "复制给ChatGPT的内容": f"这是我用“人民日报考公考编学习助手V6.1”生成的学习包，任务码是：{task_code}。请读取我上传的Markdown或Excel内容，优先分析精品文章前{top_n}篇。注意：不要简单总结，要按照深度精读要求分析，先讲清原文关键内容，让我不看原文也能知道文章讲了什么；再分析深层含义和政策背景；再按公考文章结构拆解；再做申论、面试、机关思维、工业视角、行测言语、中英表达和金句积累。完成解析后，请把不同文章分别做成Word文档，并把所有Word打包成一个zip文件返回给我下载。",
            "说明": "把Markdown分析包或Excel上传到当前ChatGPT窗口后，复制这一句话即可。",
        },
        {
            "序号": 2,
            "用途": "只分析最高分文章",
            "复制给ChatGPT的内容": f"请读取我上传的文件，任务码：{task_code}。只分析考公价值分最高的1篇文章。要求不是简单总结，而是做深度精读：原文关键内容、深层含义、政策背景、文章结构、申论积累、面试思维、机关思维、工业视角、行测言语、中英表达、金句积累。完成后请生成一个Word文档返回。",
            "说明": "第一次测试或手机端快速使用时用这个。",
        },
        {
            "序号": 3,
            "用途": "让ChatGPT帮我回填表格",
            "复制给ChatGPT的内容": "请把你的解析结果整理成表格，字段包括：标题、原文关键内容、深层含义、结构拆解、申论主题、申论规范表达、面试题、面试答题思路、机关思维、工业视角积累、行测言语积累、中英表达、金句积累。",
            "说明": "解析完之后追加这句话，让结果更方便复制回Excel。",
        },
        {
            "序号": 4,
            "用途": "V6模式说明",
            "复制给ChatGPT的内容": "V6.1不调用任何API，只生成可上传给ChatGPT的Markdown、Excel和Word学习包。请直接根据我上传的文件做深度精读分析。",
            "说明": "防止混淆。",
        },
    ])
...
def build_chatgpt_package(df: pd.DataFrame, task_code: str, top_n: int = 5) -> pd.DataFrame:
    ...
    for _, row in top_df.iterrows():
        ...
        rows.append({
            ...
            "链接": row_dict.get("链接", ""),
            "请ChatGPT输出": "原文关键内容精读、深层含义、政策背景、文章结构、公考阅读逻辑、申论积累、面试思维、机关思维、工业视角、行测言语、中英表达、金句积累，并按文章分别生成Word后打包zip",
        })
    return pd.DataFrame(rows)
...
def build_chatgpt_markdown_package(df: pd.DataFrame, task_code: str, top_n: int = 5) -> str:
    ...
    top_df = df.sort_values(by="考公价值分", ascending=False).head(top_n)
    lines = []
    lines.append("# 人民日报考公考编学习助手 V6.1 Markdown分析包")
    lines.append("")
    ...
    lines.append("")
    lines.append("> 请读取我上传的 Markdown 文件，按照文件内置的深度精读指令，优先分析精品文章。不要简单总结，要讲清原文关键内容、深层含义、政策背景、文章结构、申论、面试、机关思维、工业视角、行测言语、中英表达和金句积累。完成解析后，请把不同文章分别生成Word文档，并把所有Word打包成一个zip文件返回给我下载。")
    lines.append("")
    ...
    lines.append("4. 做申论、面试、机关思维、工业视角、行测言语、中英表达和金句积累。")
